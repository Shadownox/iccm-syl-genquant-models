...
PARAM_BOUNDS = [[0.0, 1.0], [3.0, 8.0], [0.0, 1.0], [0.0, 1.0]]
...
def source_path(mreas_path='.mreasoner'):
    ...
    # Manually define mReasoner directory for testing
    mreas_path = '.mreasoner/mReasoner'
...
class MReasoner():
    ...
    def query(self, premises, param_dict=None, count=0):
        if param_dict == None:
        ...
        self._send(cmd)
        out = self.wait_for_output('Conclusion: ', timeout=1)
        ...
        if out is None:
            ...
            # Just don't use these settings if mReasoner cannot find a solution after 20 tries
            if count == 10:
                self.logger.warn('REQUEST ABANDONED: Parameter settings are not considered')
                return []

            # Reinitialize current mReasoner instance after 5 tries
            if count == 5 or count == 10 or count == 15:
                self.terminate()
                self.initialize()

            # Try again
            return self.query(premises, param_dict=param_dict, count=count+1)
