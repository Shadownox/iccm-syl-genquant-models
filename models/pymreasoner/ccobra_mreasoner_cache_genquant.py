...
class CCobraMReasoner(ccobra.CCobraModel):
    ...
    def __init__(self, name='mReasoner', n_samples=10, fit_its=6, cache_file=None):
        ...
        super(CCobraMReasoner, self).__init__(name, ['syllogistic', 'syllogistic-generalized'], ['single-choice'])
    ...
    def fit(self):
        ...
        for idx_epsilon, p_epsilon in enumerate(np.linspace(*mreasoner.PARAM_BOUNDS[0], self.fit_its)[:5]):
            for idx_lambda, p_lambda  in enumerate(np.linspace(*mreasoner.PARAM_BOUNDS[1], self.fit_its)):
                for idx_omega, p_omega in enumerate(np.linspace(*mreasoner.PARAM_BOUNDS[2], self.fit_its)):
                    for idx_sigma, p_sigma in enumerate(np.linspace(*mreasoner.PARAM_BOUNDS[3], self.fit_its)):
                        ...
                        pred_mat = self.prediction_cache[idx_epsilon, idx_lambda, idx_omega, idx_sigma]

                        # Compare predictions with data (old way of calculating score)
                        pred_mask = (pred_mat == pred_mat.max(axis=1, keepdims=True))
                        score = np.sum(np.mean(train_data * pred_mask, axis=1))

                        # New way of calculating score
                        #score = np.sum(train_data * pred_mat)
    ...
    def predict(self, item, **kwargs):
        ...
        syl_idx = self.SYLLOGISMS_gen.index(syllog.encoded_task)
        preds = pred_mat[syl_idx]

        # Determine best prediction
        cand_choices = np.array(self.RESPONSES_gen)[preds == preds.max()]
